...
const catalog = JSON.parse(fs.readFileSync(new URL('./weapon-catalog.json', import.meta.url), 'utf8'));
...
export function classifyWeaponFeature(feature, { limit = 5 } = {}) {
  if (!feature) return { status: 'unavailable', confidence: 0, candidates: [] };
...
export const weaponCatalogMetadata = { version: catalog.version, source: catalog.source, count: templates.length };
