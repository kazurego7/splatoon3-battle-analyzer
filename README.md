...
実装済みなのは、自動試合分割、実動画再生、自分のデス検出、敵味方の生存人数と人数差、分析ポイント、動画・シークバー・グラフの同期です。画面上部HUDを0.25秒刻みで追跡し、タイマーが見えている場面だけから灰色の×表示を検出します。ゲームカウントOCR、プレイヤーや敵味方の位置・動線はまだ解析データを生成できないため、画面でも未対応と明示しています。モックは `../mock/` に参照用として保存しています。
