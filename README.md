...
6. サムネイル、画面変化、本人HUD枠、復活UI、自分のデス、生存人数、ゲームカウント、観測マップ、分析候補を生成する
...
実装済みなのは、自動試合分割、実動画再生、リザルトからの本人HUD枠特定、右下の復活カウントUIによるデス確認、敵味方の生存人数と人数差、ゲームカウント、映像内マップ画面の抽出、`../assets/stage-maps/` のステージ素材表示、分析ポイント、動画・シークバー・グラフ・マップの同期です。本人HUD枠は固定座標で決めず、リザルトの自分行に表示されたブキ画像を4つの自軍HUDアイコンへ照合します。映像内の敵味方検出、ブキ名、自分の試合全体の動線、知覚情報からの予想動線はまだ解析データを生成できないため、観測できていない位置を画面へ作らない方針です。モックは `../mock/` に参照用として保存しています。
