...
実装済みなのは、自動試合分割、実動画再生、リザルトからの本人HUD枠特定、上部HUDと右下の復活カウントUIを使ったデス時刻照合、敵味方の生存人数と人数差、ゲームカウント、映像内マップ画面の抽出、`../assets/stage-maps/` のステージ素材表示、分析ポイント、動画・シークバー・グラフ・マップの同期、自分と味方の観測・短期予測動線です。本人HUD枠は固定座標で決めず、リザルトの自分行に表示されたブキ画像を4つの自軍HUDアイコンへ照合します。デス時刻は両UIの早い検出を採用し、上部HUDが隠れている場合は復活UIの初出へフォールバックします。デス直前の通常戦闘では、相手色・形状・複数フレームの動きが連続する領域だけを低確度の敵候補として破線表示します。敵候補と自分の推定動線が重なる時刻は、自分の移動方向を仮のカメラ方向として、マップ上に広い不確実範囲と4秒の予測動線を表示します。敵味方の確定検出、ブキ名、実カメラ姿勢による正確な座標対応は未完成のため、観測と予測を分離する方針です。モックは `../mock/` に参照用として保存しています。
