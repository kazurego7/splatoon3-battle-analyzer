...
- `CODEX_DEATH_ANALYSIS=true`: AIによるデス画像分析を有効化する（既定は無効）
...
リザルトのデス数読取は常にローカルで完結し、CodexやOpenAI APIを呼びません。AIによるデス説明も既定では実行しません。`CODEX_DEATH_ANALYSIS=true` を明示し、Codex CLIへ **ChatGPTアカウントでログインしている場合だけ** 実行します。`npx codex login status` で状態を確認できます。親プロセスに `OPENAI_API_KEY` が設定されていても解析プロセスへ渡さないため、OpenAI APIの従量課金へ自動で切り替わることはありません。ChatGPTプランの利用上限へ達した場合は、自動説明をスキップしてローカル判定結果を表示します。
