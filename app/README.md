...
試合画面のデス分析は、失敗パターンを1〜2文にまとめたレポートダイジェスト、その下に時刻とデス内容が分かる縦一覧を表示します。一覧はスクロールでき、項目を選ぶと映像の8秒前へ移動します。長い反復パターン分析は「俯瞰レポート」で別ウィンドウに開きます。各パターンは独立した動画プレイヤーを持ち、AIが選んだ複数の根拠範囲を順にループする `クリップループ` と、元動画を制限なく見る `通常再生` を個別に切り替えられます。
