...
試合画面のデス分析は、未分析時にはレポートダイジェスト欄に分析内容の説明と `AI分析` ボタンを表示し、分析後は失敗パターンを1〜2文にまとめたダイジェストと `俯瞰レポート` ボタンへ切り替えます。分析中に一覧へ戻っても、同じ試合を開き直すと進行中表示を復元します。その下には時刻とデス内容だけの縦一覧を表示します。一覧はスクロールでき、項目を選ぶと映像の8秒前へ移動します。長い反復パターン分析は「俯瞰レポート」で別ウィンドウに開きます。各パターンは独立した動画プレイヤーを持ち、AIが選んだ複数の根拠範囲を順にループする `クリップループ` と、元動画を制限なく見る `通常再生` を個別に切り替えられます。時系列グラフへカーソルを合わせると、その時刻の両軍カウントとペナルティを表示します。
