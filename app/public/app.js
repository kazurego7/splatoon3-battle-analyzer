...
function backToAnalysisList() { currentAnalysis=null;elements.video.pause();elements.video.removeAttribute('src');elements.reviewView.hidden=true;setReviewMode(false);renderSelected(); }
...
function matchCard(recording,match) {
  ...
  const setFacts=analysis=>{const counts=analysis.gameFlow?.gameCounts||[],last=counts.at(-1),outcome=!last?'未判定':last.teamCount<last.enemyCount?'WIN':last.teamCount>last.enemyCount?'LOSE':'DRAW',stage=analysis.stageMap?.stage||'ステージ未判定',rule=analysis.stageMap?.rule||'ルール未判定',weapon=analysis.playerIdentity?.weapon,weaponName=weapon?.status!=='candidate-only'&&weapon?.name?weapon.name:'ブキ未判定';facts.innerHTML='';[[outcome,'result'],[`${stage} / ${rule}`,'stage'],[weaponName,'weapon']].forEach(([text,className])=>{const item=document.createElement('span');item.className=`match-fact ${className}${text==='WIN'?' win':text==='LOSE'?' lose':''}`;item.textContent=text;facts.append(item);});};
...
function selectDeathEvent(event,key,{seek=true}={}) {
  ...
  if(seek){elements.video.currentTime=Math.max(0,event.time-8);updatePlaybackUi();}
}
...
async function openMatch(recording,match) {
  ...
  renderChart();renderMapBase();await loadPositionPlan();renderEvents();updatePlaybackUi();
}
