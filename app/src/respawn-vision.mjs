...
export function detectRespawnRuns(samples, { gameplayStart = 10, gameplayEnd = Infinity } = {}) {
  ...
  for (const sample of observations) {
    ...
    const gap = previous ? sample.time - previous.end : Infinity;
    if (previous && gap <= 0.75) {
      previous.end = sample.time;
      previous.peak = sample.score > previous.peak.score ? sample : previous.peak;
      previous.observations += 1;
      previous.consecutiveObservations = gap <= 0.35 ? previous.consecutiveObservations + 1 : 1;
      previous.maxConsecutiveObservations = Math.max(previous.maxConsecutiveObservations, previous.consecutiveObservations);
    } else {
      runs.push({
        start: sample.time,
        end: sample.time,
        ...
        maxConsecutiveObservations: 1,
      });
    }
  }
  return runs
    // The countdown remains visible for several seconds. Short or intermittent
    // matches are usually opening music text, ink highlights, or other HUD text.
    .filter(run => run.maxConsecutiveObservations >= 8)
    .map((run, index) => ({
      id: `death-${index + 1}`,
      time: run.start,
      end: run.end + 0.5,
      ...
      evidence: {
        ...
        consecutiveObservations: run.maxConsecutiveObservations,
      },
    }));
}
