...
function latestStateBefore(states, time, maxAge = 3) {
  return [...(states || [])]
    .reverse()
    .find(state => state.time <= time && time - state.time <= maxAge) || null;
}

export function describeSelfDeaths(deaths, { detections = [], playerCounts = [] } = {}) {
  return deaths.map(death => {
    const enemyMotion = detections.find(detection =>
      detection.kind === 'enemy-color-motion-candidate'
      && detection.evidence?.deathId === death.id);
    const weaponName = typeof enemyMotion?.weapon === 'string'
      ? enemyMotion.weapon
      : enemyMotion?.weapon?.name || null;
    const players = latestStateBefore(playerCounts, death.time - 0.25);
    const situationParts = [];

    if (players) {
      const balance = players.difference > 0
        ? `${players.difference}枚有利`
        : players.difference < 0
          ? `${Math.abs(players.difference)}枚不利`
          : '人数は五分';
      situationParts.push(`デス直前は自軍${players.teamAlive}人・相手${players.enemyAlive}人（${balance}）`);
    }
    if (enemyMotion) situationParts.push('デス前4秒以内に画面上で敵色の動き候補を検出');

    const title = weaponName
      ? `${weaponName}の攻撃を受けて倒された`
      : enemyMotion
        ? '敵候補との交戦で倒された'
        : '攻撃方法を特定できないデス';
    const cause = weaponName
      ? `${weaponName}の攻撃を受けた可能性があります。映像上の候補に基づくため、確定情報ではありません。`
      : enemyMotion
        ? 'デス直前に敵色の形状と動きが連続していたため、敵との交戦が原因の可能性があります。攻撃方法とブキは未特定です。'
        : 'デスしたことは確認できましたが、倒した相手・攻撃方法・ブキは映像から特定できませんでした。';

    return {
      ...death,
      title,
      situation: situationParts.length
        ? `${situationParts.join('。')}。`
        : 'デス直前の人数・カウント状況は判定できませんでした。',
      cause,
    };
  });
}
