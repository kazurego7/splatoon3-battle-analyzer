...
function renderChart() {
  ...
  elements.chartEvents.innerHTML=''; currentAnalysis.events.forEach(event=>{const x=event.time/duration*1000;const line=document.createElementNS('http://www.w3.org/2000/svg','line');line.setAttribute('x1',x);line.setAttribute('x2',x);line.setAttribute('y1','24');line.setAttribute('y2','205');line.setAttribute('class',event.type==='death'?'chart-death':'chart-event');elements.chartEvents.append(line);if(event.type==='death'){const marker=document.createElementNS('http://www.w3.org/2000/svg','polygon');marker.setAttribute('points',`${x},15 ${x+7},22 ${x},29 ${x-7},22`);marker.setAttribute('class','chart-death-marker');elements.chartEvents.append(marker);}});
}
