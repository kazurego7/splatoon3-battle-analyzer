...
function renderVideoDetections(time) {
  ...
  detections.forEach(item=>{const values=interpolate(item.frames,time);if(!values)return;const [x,y,width,height]=values;const normalized=x<=1&&y<=1;const px=normalized?x*1920:x,py=normalized?y*1080:y,pw=normalized?width*1920:width,ph=normalized?height*1080:height;const group=svgElement('g');group.append(svgElement('rect',{x:px,y:py,width:pw,height:ph,rx:8,class:`detection-box ${item.team} ${item.kind==='death-camera-focus-candidate'?'candidate':''}`}));const text=item.kind==='death-camera-focus-candidate'?'敵候補｜デスカメラ':`${item.team==='enemy'?'敵':'味方'}｜${item.weapon||'ブキ未特定'}`;group.append(svgElement('rect',{x:px,y:Math.max(0,py-38),width:Math.max(170,text.length*28),height:38,rx:6,class:'detection-label-bg'}));const label=svgElement('text',{x:px+10,y:Math.max(27,py-11),class:'detection-label'});label.textContent=text;group.append(label);elements.videoDetections.append(group);});
  elements.perceptionState.textContent=detections.length?detections.some(item=>item.kind==='death-camera-focus-candidate')?'デスカメラで敵候補を確認':`映像で敵味方 ${detections.length}件を観測`:'映像内の認識情報のみ表示';
}
...
function renderSpatialState(time) {
  ...
  for(const prediction of predictions.filter(item=>item.team!=='self')){const position=interpolate(prediction.frames,time);if(position)elements.entityLayer.append(svgElement('circle',{cx:position[0],cy:position[1],r:15,class:`map-predicted-position ${prediction.team}`}));}
